...
/* -------------------------
   Catégorie courant (1 seul)
--------------------------*/
const currentTag = computed(() => props.rows[0]);
...
watch(
  ...
  (tag) => {
    ...
    state.designation = tag.designation;
  },
  { immediate: true }
);
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    await $fetch("/api/tag/updatetag", {
      method: "PATCH",
      body: {
        id: currentTag.value.id_tag,
    ...
    toast.add({
      title: "Succès",
      description: `Catégorie « ${event.data.designation} » mis à jour`,
      color: "success",
    });
...
<template>
  <UModal v-model:open="open" title="Modifier la catégorie" :description="`Modifier les informations de la catégorie sélectionnée.`">
    <slot />
